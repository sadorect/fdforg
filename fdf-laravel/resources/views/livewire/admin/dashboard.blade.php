<div class="space-y-6">
    @php
        $adminUser = auth()->user();
        $canManagePages = $adminUser?->hasPermission(\App\Support\AdminPermissions::MANAGE_PAGES);
        $canManageEvents = $adminUser?->hasPermission(\App\Support\AdminPermissions::MANAGE_EVENTS);
        $canManageBlog = $adminUser?->hasPermission(\App\Support\AdminPermissions::MANAGE_BLOG);
        $canManageCategories = $adminUser?->hasPermission(\App\Support\AdminPermissions::MANAGE_CATEGORIES);
        $canManageCourses = $adminUser?->hasPermission(\App\Support\AdminPermissions::MANAGE_COURSES);
        $canManageLessons = $adminUser?->hasPermission(\App\Support\AdminPermissions::MANAGE_LESSONS);
        $canManageUsers = $adminUser?->hasPermission(\App\Support\AdminPermissions::MANAGE_USERS);
        $canManageEnrollments = $adminUser?->hasPermission(\App\Support\AdminPermissions::MANAGE_ENROLLMENTS);
        $hasAssignedRoles = $adminUser?->roles()->exists() ?? false;
    @endphp
    <!-- Page Header -->
    ...
    @if($adminUser?->is_admin && ! $adminUser->isSuperAdmin() && ! $hasAssignedRoles)
        <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            <p class="font-semibold">No admin roles assigned yet.</p>
            <p class="mt-1">This account can sign in, but it will only see basic admin pages until an access role is assigned.</p>
        </div>
    @endif
